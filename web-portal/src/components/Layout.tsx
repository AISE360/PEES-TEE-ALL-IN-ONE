...
const nav = [
  ...
  {label:"Premium Applications", to:"/premium"},
  {label:"Employees", to:"/employees"},
  {label:"HR & Leave", to:"/hr-leave"},
  {label:"Field Tracking", to:"/field-tracking"},
...
export default function Layout({children}:{children:React.ReactNode}){
  ...
  useEffect(()=>{
    ...
    s.on("new_client_request", (p:any)=>{ setToasts(t=>[...t, `New quote: ${p.serviceType} #${p.reference}`]); setBell(b=>b+1); setTimeout(()=>setToasts(t=>t.slice(1)), 4000); });
    s.on("new_premium_quote", (p:any)=>{ setToasts(t=>[...t, `New KYC: ${p.referenceNo}`]); setBell(b=>b+1); });
    s.on("stage_change", (p:any)=>setToasts(t=>[...t.slice(-2), `Stage: ${p.reference} → ${p.stage}`]));
    return ()=>{ s.disconnect(); };
  },[]);
  return (
    <div className="min-h-screen flex">
      <aside className="w-[240px] bg-[#0F2440] text-white flex flex-col shrink-0">
        ...
        <nav className="p-3 flex-1 space-y-1">
          {nav.map(n=>(
            <NavLink key={n.to} to={n.to} end={n.to==="/"} className={({isActive})=>
              `block px-3 py-2.5 rounded-xl text-sm transition-colors ${isActive?"bg-[#C6A664] text-[#0F2440] font-bold":"text-white/70 hover:bg-white/10 hover:text-white"}`
            }>{n.label}</NavLink>
          ))}
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-[64px] bg-white border-b flex items-center justify-between px-6 sticky top-0 z-10 shrink-0">
        ...
        <div className="fixed top-[72px] right-6 space-y-2 z-50">
          {toasts.map((t,i)=>(<div key={i} className="bg-[#0F2440] text-white px-4 py-3 rounded-xl shadow-xl text-sm">{t}</div>))}
        </div>
